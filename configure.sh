...
if [ ! -f ext/geogram/CMakeOptions.txt.ovm_io ]; then
...
NAME
    configure.sh
SYNOPSIS
    Prepares the build environment for ovm_io.
...
PLATFORM
    Build platforms supported by ovm_io: use configure.sh --help-platforms
END
            exit
            ;;
